...
const getweather = async function (req, res) {
  ...
  try {
    ...
    let options = {
      ...
      // url : ` http://api.openweathermap.org/data/2.5/weather?q=London&appid=ffc803e155dca2e8efa4ea6cc04891e1 `,
      url: ` http://api.openweathermap.org/data/2.5/weather?q=${q}&appid=${appid} `,
    }
    let response = await axios(options)

    let id = response.data
    res.status(200).send({ msg: "The weather of London is :", data: id })
...
//2nd PROBLEM
const getweather2 = async function (req, res) {
  ...
  try {
    ...
    res.status(200).send({ msg: "The Temperature of London is :", data: id })
